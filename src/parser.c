...
static ASTNode *parse_primary_expr() {
	...
	switch (current->type) {
		case TOKEN_NUMBER: {
			...
			new_node->data.number_lit = atol(current->value); // un pelin peligroso
			break;
				   }
		case TOKEN_STRING: {
			eat();
			new_node = newASTNode(NODE_STRING_LIT);
			new_node->data.string_lit = strdup(current->value); // peligroso
			break;
				   }
		case TOKEN_DOLLAR: {
			eat();
			...
			if (current->type==TOKEN_COLON) {
				...
				new_node->data.var_ref.name = strdup(current->value);
